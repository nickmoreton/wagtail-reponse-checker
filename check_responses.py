    ...
    def report_page(self, session, options):
        ...
        for page_model in page_models:
            if item := page_model.objects.first():
                # some pages aren't assigned to a site
                if item.get_site() is not None:
                    # some pages are part of a different site that's not using localhost
                    # multisite for example
                    if options["host"] in item.get_url():
